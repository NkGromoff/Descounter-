...
import { searchGames, SetMainPageGamesIdNullReduserCreator, SetMainPageSearchGames } from "../../../redux/MainPageReduser";
...
const Banners = React.memo((props) => {
    ...
    const openAllList = (e) => {
        if (e.key === "Enter") history.push(gamesSearchDisplay.length > 0 ? `/allGames?term=${e.target.value}` : "/allGames");
    };
                                ...
                                <div className="gameGallery__searchWrapper">
                                    <div className={`gameGallery__searchInner ${inputShow ? "gameGallery__searchInner--active" : ""}`}>
                                        <Field
                                            ...
                                            type="text"
                                            onKeyPress={openAllList}
                                            name="term"
                                            className="gameGallery__serchInp"
                                            placeholder="Поиск"
                                            innerRef={seacrhRef}
                                        />
                                        <div className={`gameGallery__serchItemWrapper ${listShow ? "gameGallery__serchItemWrapper--active " : ``}`}>
                                            ...
                                            <NavLink to={gamesSearchDisplay.length > 0 ? `/allGames?term=${values.term}` : "/allGames"} className="gameGallery__serchLink">
                                                Весь список
                                            </NavLink>
                                        </div>
                                    </div>
